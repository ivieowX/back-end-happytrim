...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO diet_dinner (name, calorie, diet_image, details, date) VALUES (?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($dbcon, $sql);
    mysqli_stmt_bind_param($stmt, "sisss", $name, $calorie, $diet_image, $details, $date);

    if (mysqli_stmt_execute($stmt)) {
        // เพิ่มการแจ้งเตือนเมื่อเพิ่มอาหารเย็นใหม่
        $notification_title = "มีอาหารเย็นใหม่";
        $notification_message = "อาหารเย็นใหม่: " . $name;
