...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO diet_lunch (name, calorie, diet_image, details, date) VALUES (?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($dbcon, $sql);
    mysqli_stmt_bind_param($stmt, "sisss", $name, $calorie, $diet_image, $details, $date);

    if (mysqli_stmt_execute($stmt)) {
        // เพิ่มการแจ้งเตือนเมื่อเพิ่มอาหารกลางวันใหม่
        $notification_title = "มีอาหารกลางวันใหม่";
        $notification_message = "อาหารกลางวันใหม่: " . $name;
